...
try {
  ...
  assert(assistantDrawer.includes("state.admin.user?.avatar || publicUrl('/assets/default_avatar.jpg')") && assistantDrawer.includes("adminDisplayName"), "assistant user messages prefer the signed-in user's uploaded avatar and fall back to the default image");
  ...
  assert(assistantDrawer.includes('class="assistant-source-button"') && assistantDrawer.includes(".assistant-source-button :deep(span)") && assistantDrawer.includes("overflow-wrap: anywhere") && assistantMessageContent.includes("word-break: break-word"), "assistant long source URLs, Markdown links, and table cells wrap within the message width");
  assert(assistantDrawer.includes("formatDateTimeWithYear(item.updatedAt)") && assistantDrawer.includes("formatDateTimeWithYear(message.createdAt)") && assistantDrawer.includes("复制问题") && assistantDrawer.includes("复制回答"), "assistant conversation and message history show persisted timestamps with copy actions for both roles");
