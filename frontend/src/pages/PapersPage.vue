    ...
    <el-card shadow="never" class="paper-list-card">
      ...
      <el-table :data="pagedPaperRows" class="mt-4 w-full" empty-text="暂无匹配试卷" @row-dblclick="onRowDoubleClick">
        ...
        <el-table-column label="操作" fixed="right" width="160" align="right">
          <template #default="{ row }">
            <el-button v-if="row.status === '已发布'" link type="primary" :icon="View" @click="selectPaper(row.id)">查看</el-button>
            <el-button v-else link type="primary" :icon="Edit" @click="editPaper(row)">编辑</el-button>
