...
- AI 命题：配置试卷名称、出题方向、难度、题型、题量、分值和知识点
- 出题资料：管理文本、TXT、Markdown、PDF 和 DOCX 资料，支持多选资料及资料题/AI 生成题配额
- 题库管理：支持人工建题、试卷题目入库、自动去重、版本记录和试卷使用关系
- 题目复用：同一道题从多套试卷入库时只保留一条题库记录，各试卷仍保存独立题目快照
- 来源追溯：资料题保存资料版本和引用片段，人工修改及历史试卷仍保留来源记录
- 生成稳定性：规范化 AI 输出并校验题量、题型、总分和必填字段
- 题目质量：发布时自动检查题目结构、答案和评分规则，并支持人工编辑修正
...
默认账号可以直接登录。正式部署首次启动前，请通过环境变量设置独立的初始控制台账号和密码。
